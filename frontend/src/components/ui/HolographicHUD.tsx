        ...
        <aside className="hologram-card p-8 w-80 pointer-events-auto animate-in fade-in slide-in-from-left-10 duration-1000 delay-300">
          ...
          <div className="space-y-6">
            ...
            <div className="pt-2 space-y-3">
              ...
              <button
                onClick={() => useStudoStore.getState().setScene({ isSimulating: !scene.isSimulating })}
                className={`neon-btn w-full ${scene.isSimulating ? 'bg-neon-cyan/30 text-white border-neon-cyan' : ''}`}
              >
                <svg className={`w-4 h-4 ${scene.isSimulating ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                {scene.isSimulating ? 'Stop Physics' : 'Run Simulation'}
              </button>
            </div>
